(function (factory) {
    ...
    function DateTimePicker(element, options) {
        var picker = {},
            ...
            _createCellDev = function ($tr, text, nav, cls) {
                var $cell = $('<span tabindex="0">')
                    .html(text)
                    .appendTo($tr);
                if (nav) {
                    $cell.data('nav', nav);
            ...
            _createDatePicker = function () {
                ...
                //head - tools - title
                $table.$title = $('<td class="title" colspan="5" tabindex="0"/>').data('nav', NAV['title']).appendTo($row);
                ...
                for (i = 6; i > 0; i--) {
                    ...
                    for (var t = 0; t < 7; t++) {
                        $('<td tabindex="0"/>').appendTo($bodyRow);
                    }
                }
                return $table;
            },
            ...
            _createMonthPicker = function () {
				...
				$('<td class="head-month" colspan="4"/><td class="division"/>').appendTo($row);
                _createCell($row, '<i class="icon-datepicker-prev"/>', 1, NAV['prevy'], ' prevy');
				$('<td class="head-year" colspan="2"/>').appendTo($row);
				...
				$('<td class="division"/>').appendTo($row);*/
                //tbody - 翻年按钮
                //tbody - years
                for (var m = 0; m < 6; m++) {
                    $row = $('<tr/>').appendTo($tbody);
                    $('<td class="month" colspan="2" tabindex="0"/><td class="month" colspan="2"time-btn/><td class="division"/>' +
                        '<td class="year" colspan="2" tabindex="0"/><td class="year" colspan="2" tabindex="0"/>').appendTo($row);
                }
                //foot - buttons
                var $tfoot = $('<tfoot class="buttonpane"/>').appendTo($table);
                $row = $('<tr/>').appendTo($tfoot);
                var $rowTf = $('<td colspan="9"/>').appendTo($row);
                //tbody - 确定与取消
                _createCellDev($rowTf, I18N["OK"], NAV['mok'], 'ok');
                return $table;
            },
            ...
            _createButtonPane = function () {
                var $buttonpane = $('<div class="buttonpane"/>');
                var $row = $('<div>');
                // clear button
                _createCellDev($row, I18N["CLEAR"], NAV['clear'], 'clear');
                // today button
                _createCellDev($row, I18N["TODAY"], NAV['today'], 'today');
                // ok button
                _createCellDev($row, I18N["OK"], NAV['dok'], 'ok');
            ...
            _toPrevDecade = function () {
                var sd = options.startDate, date = currentDate;
                var year = date.getFullYear() - 12, month = date.getMonth();
            ...
            _loadDateData = function (table, date) {
                ...
                var std = options.startDate,
                    edd = options.endDate;
                //set title
                table.$title.text(year + ", " + I18N.MN[month]);
                //set button
            ...
            _loadMonthData = function (table, date) {
                ...
                var years = [midyear - 5, midyear - 4, midyear - 3, midyear - 2, midyear - 1, midyear,
                    midyear + 1, midyear + 2, midyear + 3, midyear + 4, midyear + 5, midyear + 6];
                var ycells = $("td.year", table);
                var mcells = $("td.month", table);
				var ytext = $("td.head-year", table);
				var mtext = $("td.head-month", table);
            ...
            _createTimePicker = function (viewmode) {
                ...
                $table.focus = $table.$m;
                var $add = $('<td class="time-btn"time-btn tabindex="0"/>').append($('<i class="icon-datepicker-plus"/>')).data('nav', NAV['plus']);
                var $min = $('<td class="time-btn"time-btn tabindex="0"/>').append($('<i class="icon-datepicker-minus"/>')).data('nav', NAV['minus']);
                var $row = $('<tr/>').append($('<td class="time-text" rowspan="2"/>').text(I18N.TIME))
            ...
            _bindEvts = function () {
                var proxy = function (event) {
                    ...
                    var $target = $(target).closest('td, .buttonpane span');
                    var type = event.type;
                    ...
                    } else if (type === "mouseup" || event.keyCode == 13) {
                        ...
                        switch (navitype) {
                            ...
                            case NAV['title']:
                                ...
                                $monthtable.css({
									...
									$parent.addClass('month-division');
									$parent.find('.mt .prevy').focus();
								});
                                break;
                            case NAV['clear']:
                                //清空按钮
                            ...
                            case NAV['mok']:
                                ...
                                if ($datetable.parent().length > 0) {
									$monthtable.parent('.perfect-datetimepicker').removeClass('month-division');
                                    $monthtable.hide("fast",function(){
										$monthtable.parent('.perfect-datetimepicker').find('.dt .prevm').focus();
									});
                                }
                                break;
                            case NAV['cancel']:
